<template>
    <div class="p-6 space-y-4">
        <div v-if="status !== 'pending'" class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <StudentInfoCard icon="i-lucide-clock" label="In Progress"
                :numberOfItems="data?.coursesInProgress.length ?? 0" />
            <StudentInfoCard label="Completed" variant="success" icon="i-lucide-check-circle"
        ...
        <div>
            <div class="grid gap-4 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-4">
                <template v-if="status !== 'pending' && data?.allCourses">
                    <NuxtLink v-for="course in data?.allCourses" :key="course.id" :to="`/courses/${course.id}`">
            ...
            <div class="mt-10 text-sm text-center text-muted-foreground" v-if="data?.allCourses && !data?.allCourses.length">
                No courses found
            </div>
        </div>
    </div>
...
const { data, status } = await useFetch(`/api/courses/usercourses`);
</script>
